...
wheel_left: Motor = robot.getDevice("wheel2 motor")
wheel_right: Motor = robot.getDevice("wheel1 motor")
wheel_left.setPosition(float('inf'))
wheel_right.setPosition(float('inf'))

camera_left: Camera = robot.getDevice("camera3")
camera_middle: Camera = robot.getDevice("camera1")
camera_right: Camera = robot.getDevice("camera2")
camera_left.enable(timestep)
camera_middle.enable(timestep)
...
def main():
    ...
    while robot.step(timestep) != -1:
        ...
        if step_counter % interval == 0:
            for camera in [camera_left, camera_middle, camera_right]:
