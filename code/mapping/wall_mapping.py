...
class WallMapper:
    def __init__(self, mixed_grid: MixedGrid, robot_diameter: float) -> None:
        ...
        # Perimeter with the radius of the robot
        self.robot_diameter_template = np.zeros((self.robot_diameter, self.robot_diameter), dtype=np.uint8)
        self.robot_diameter_template = cv.circle(self.robot_diameter_template, (self.robot_radius, self.robot_radius),
                                                 self.robot_radius, 255, -1)
        self.robot_diameter_template = self.robot_diameter_template.astype(np.bool_)

        # A template to calculate the preference of each pixel for navigation taking into account the distance from
        # the wall
    ...
    def load_point_cloud(self, in_bounds_point_cloud, out_of_bounds_point_cloud, robot_position):
        ...
        Calculates the traversable areas and the preference of each point for navigation.
        """
    ...
    def calculate_seen_walls(self):
        ...
        self.grid.arrays["walls_not_seen_by_camera"] = np.logical_xor(self.grid.arrays["walls"],
    ...
    def occupy_point(self, point_array_index):
        if not self.grid.arrays["walls"][point_array_index[0], point_array_index[1]]:
            ...
            if self.grid.arrays["detected_points"][
                point_array_index[0], point_array_index[1]] > self.to_boolean_threshold:
    ...
    def mark_point_as_seen_by_lidar(self, robot_array_index, point_array_index):
        self.grid.arrays["seen_by_lidar"] = self.__draw_bool_line(self.grid.arrays["seen_by_lidar"], robot_array_index,
                                                                  point_array_index)
